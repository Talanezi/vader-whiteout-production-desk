...
exports.buildCallSheetLatex = buildCallSheetLatex;
function esc(value) {
    return String(value ?? '')
...
function makeTableRow(values) {
    return `${values.join(' & ')} \\\\`;
}
function padRows(rows, total, cols) {
    const out = [...rows];
...
function buildCallSheetLatex(draft) {
    const emergencyRows = draft.emergencyContacts.length
        ? draft.emergencyContacts.map((c) => `${esc(c.label)} & ${esc(c.name)} \\hfill \\mbox{${esc(c.phone)}} \\\\`)
        : [' &  \\\\'];
    const sceneRows = padRows(draft.scenes.map((s) => makeTableRow([
        esc(s.sceneNumber),
    ...
    const castRows = padRows(draft.castCalls.map((c) => makeTableRow([
        esc(c.castName),
        esc(c.roleName),
        esc(c.email),
        esc(c.callTime),
    ...
    const crewRows = draft.crewCalls.length
        ? draft.crewCalls.map((c) => makeTableRow([
...
//# sourceMappingURL=build-callsheet-latex.js.map