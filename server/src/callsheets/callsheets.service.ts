...
export class CallsheetsService {
  ...
  private normalizeDraft(id: string, payload?: Partial<CallSheetDraft>): CallSheetDraft {
    return {
      ...
      emailSubject: payload?.emailSubject || `Vader: Whiteout Call Sheet${payload?.productionDate ? ` - ${payload.productionDate}` : ''}`,
      emailPreheader: payload?.emailPreheader || 'Call sheet and production files for the next shoot day.',
      emailHeadline: payload?.emailHeadline || 'Call Sheet Ready for Review',
      emailIntro: payload?.emailIntro || payload?.distributionMessage || '',
      ...
      emailTransportDetails: payload?.emailTransportDetails || '',
      emailWeatherTitle: payload?.emailWeatherTitle || 'Weather',
      emailWeatherDetails: payload?.emailWeatherDetails || payload?.weatherSummary || '',
      emailSetTitle: payload?.emailSetTitle || 'Set',
      emailSetDetails: payload?.emailSetDetails || payload?.mainSetName || '',
